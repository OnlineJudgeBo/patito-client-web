...
$sql="SELECT 
	users.user_id,users.nick,users.lastname,solution.result,solution.num,solution.in_date 
		FROM 
...
echo "<table border=1><tr><td>Puesto<td>Usuario<td>Apellido<td>Nombre<td>Resuelto<td>Mark";
for ($i=0;$i<$pid_cnt;$i++)
	echo "<td>$PID[$i]";
echo "</tr>";
...
if(strpos($_SERVER['HTTP_USER_AGENT'],'MSIE')){
                $U[$i]->lastname=iconv("utf8","gbk",$U[$i]->lastname);
        }
        echo "<td>".($U[$i]->lastname)."";
	echo "<td>".$U[$i]->nick."";
	echo "<td>$usolved";
	echo "<td>";
