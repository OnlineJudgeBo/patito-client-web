...
<script>
    const pendingSubmissions = document.querySelectorAll('#status-table .pending');
    const refreshKey = `status-refresh:${window.location.pathname}${window.location.search}`;
    const refreshAttempts = Number(sessionStorage.getItem(refreshKey) || 0);
    const maxRefreshAttempts = 10;

    if (pendingSubmissions.length > 0 && refreshAttempts < maxRefreshAttempts) {
        sessionStorage.setItem(refreshKey, String(refreshAttempts + 1));
...
function rejudgeSolution(solutionId) {
    ...
    fetch(`https://juezvirtual.com.bo/api/Judge/rejudge/solution/${solutionId}`, {
        method: 'GET',
        headers: {
