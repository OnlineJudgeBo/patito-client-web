...
class IcpcContestRepository implements IIcpcContestRepository
{
    private $pdo;
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM icpc_contest WHERE contest_id = :cid");
    ...
    {
        $stmt = $this->pdo->query("SELECT * FROM icpc_contest ORDER BY contest_id");
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function getProblemTitleByNumber($cid, $num)
    {
        ...
        FROM icpc_contest_problem, problem
