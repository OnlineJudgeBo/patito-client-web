  ...
  <body class="orange lighten-5 orange-text text-darken-4" onload="cargar()">
    ...
    <div id="main">
      <center>
        ...
        if($OJ_EDITE_AREA){
          ?>
          <script language="Javascript" type="text/javascript" src="edit_area/edit_area_full.js">
          </script>
	...
	>
        <?php if (isset($id)){
          echo "<h3>Problema $id </h3>";
          echo "<input id='problem_id' type='hidden' value=$id name='id' ><br>";
        }else{
          ...
          echo "<h3>Problema $PID[$pid] del Concurso $cid</h3>";
          echo "<input id='cid' type='hidden' value=$cid name='cid'>";
          echo "<input id='pid' type='hidden' value=$pid name='pid'>";
        }?>
	<h>Lenguaje</h5>
        <select id="language" name="language" class="browser-default orange" style="width:100px">
          <?php
        ...
        <textarea style="width:80%; font-family:'Lucida Console', Monaco, monospace" cols="180" rows="20" id="source" name="source" class="materialize-textarea white" placeholder="Inserta tu código acá."><?php echo $view_src?></textarea></br>
	 <a id="Submit" type="button" onclick=do_submit();<?php echo" $boton>$MSG_SUBMIT";?><i class='material-icons right'>send</i></a>
        <!--<input id="Submit" type="button" value=<?php echo "$MSG_SUBMIT "; echo $boton?>  onclick=do_submit();>-->
      </form>
    </center>
    <script>
     var sid=0;
