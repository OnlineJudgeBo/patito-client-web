...
function crearTablaProblemSet(){
    ...
    while ($row=mysql_fetch_object($result)){
        ...
        echo "TablaPS.body.rows[$i].row[0].textAlign=\"center\";";
        ...
        echo "TablaPS.body.rows[$i].row[2].textAlign=\"center\";";
...
function crearTablaContestSet(){
	...
	$result=mysql_query($sql);
	$i=0;
    echo "var TablaCS={props:{}, head:{props:{}, row:[]}, body:{props:{}, rows:[]}};";    
    echo "TablaCS.head.row.push({text:\"$MSG_PROBLEM_ID\"},{text:\"Nombre\"},{text:\"Estado\"},{text:\"Tipo\"});";
	while ($row=mysql_fetch_object($result)){
        echo "TablaCS.body.rows.push({props:{},row:[{},{},{},{}]});\n";
        echo "TablaCS.body.rows[$i].row[0].text=$row->contest_id;";
        echo "TablaCS.body.rows[$i].row[0].link=\"contest.php?cid=$row->contest_id\";";
        echo "TablaCS.body.rows[$i].row[0].textAlign=\"center\";";
        echo "TablaCS.body.rows[$i].row[1].text=\"$row->title\";";
        echo "TablaCS.body.rows[$i].row[1].link=\"contest.php?cid=$row->contest_id\";";
		...
		if ($now>$end_time) {// past
            echo "TablaCS.body.rows[$i].row[2].text=\"$MSG_Ended@$row->end_time\";";			
		}else if ($now<$start_time){// pending
            echo "TablaCS.body.rows[$i].row[2].text=\"$MSG_Start@$row->start_time$MSG_TotalTime".formatTimeLength($length)."\";";
            echo "TablaCS.body.rows[$i].row[2].ctext=\"green-text\";";			
		}else{// running
            echo "TablaCS.body.rows[$i].row[2].text=\"$MSG_Running $MSG_LeftTime ".formatTimeLength($left)."\";";
            echo "TablaCS.body.rows[$i].row[2].ctext=\"red-text\";";
		}
        echo "TablaCS.body.rows[$i].row[2].textAlign=\"center\";";
		$private=intval($row->private);        
		if ($private==0){
            echo "TablaCS.body.rows[$i].row[3].text=\"$MSG_Public\";";
            echo "TablaCS.body.rows[$i].row[3].ctext=\"green-text\";";
        }else{
            echo "TablaCS.body.rows[$i].row[3].text=\"$MSG_Private\";";
            echo "TablaCS.body.rows[$i].row[3].ctext=\"red-text\";";
        }
        echo "TablaCS.body.rows[$i].row[3].textAlign=\"center\";";
		$i++;
	}
	mysql_free_result($result);
}
function crearTablaContestProblemSet(){
    global $PID, $MSG_PROBLEM_ID, $MSG_TITLE, $MSG_SOURCE, $MSG_AC, $MSG_SUBMIT;
    $cid=getCid();
 ...
 order by pnum
	";
    echo "var TablaCPS={props:{}, head:{props:{}, row:[]}, body:{props:{}, rows:[]}};";    
    echo "TablaCPS.head.row.push({text:\"$MSG_PROBLEM_ID\"},{text:\"$MSG_TITLE\"},{text:\"$MSG_SOURCE\"},{text:\"$MSG_AC\"},{text:\"$MSG_SUBMIT\"});";
	$result=mysql_query($sql);
	$view_problemset=Array();
	$i=0;
	while ($row=mysql_fetch_object($result)){
        echo "TablaCPS.body.rows.push({props:{},row:[{},{},{},{},{}]});\n";        
 		$view_problemset[$i][0]="";
 		if (isset($_SESSION['user_id'])){
            if(check_ac($cid,$i)==1) echo "TablaCPS.body.rows[$i].props.bgColor=\"green\";";
            if(check_ac($cid,$i)==0) echo "TablaCPS.body.rows[$i].props.bgColor=\"red lighten-2\";";
        }
        echo "TablaCPS.body.rows[$i].row[0].text=\"Problema $PID[$i] ($row->pid)\";";
        echo "TablaCPS.body.rows[$i].row[0].link=\"problem.php?cid=$cid&pid=$i\";";
        echo "TablaCPS.body.rows[$i].row[0].textAlign=\"center\";";
        echo "TablaCPS.body.rows[$i].row[1].text=\"$row->title\";";
        echo "TablaCPS.body.rows[$i].row[1].link=\"problem.php?cid=$cid&pid=$i\";";
        echo "TablaCPS.body.rows[$i].row[1].textAlign=\"center\";";
        echo "TablaCPS.body.rows[$i].row[2].text=\"$row->source\";";
        echo "TablaCPS.body.rows[$i].row[2].textAlign=\"center\";";
        echo "TablaCPS.body.rows[$i].row[3].text=\"$row->accepted\";";
        echo "TablaCPS.body.rows[$i].row[3].textAlign=\"center\";";
        echo "TablaCPS.body.rows[$i].row[4].text=\"$row->submit\";";
        echo "TablaCPS.body.rows[$i].row[4].textAlign=\"center\";";
        $i++;
	}
	mysql_free_result($result);
}
function crearDatosContestProblemSet(){
    global $view_title;
    $cid=intval($_GET['cid']);
 	$view_cid=$cid;
    ...
    $end_time=strtotime($row->end_time);
    $order=array("\r\n", "\n", "\r");
    $des=str_replace($order, "\\n", $row->description);
    $des=str_replace("\"", "\\\"", $des);
    echo "dat.getCStart=\"$row->start_time\";";
    echo "dat.getCEnd=\"$row->end_time\";";
